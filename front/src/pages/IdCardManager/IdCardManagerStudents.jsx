...
import { Card, Row, Col, Table, Button, Spinner, Alert, Modal, Form, Badge, Image as BSImage } from 'react-bootstrap';
import {
...
const IdCardManagerStudents = () => {
    ...
    const submitPhotoUpdate = async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                loadClassStudents(); // Recharger la liste
            } else {
    ...
    const handlePrintClassCards = async () => {
        try {
            const response = await fetch(
                `${host}/api/student-cards/class/${classId}/generate`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
            ...
            if (response.ok) {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = `cartes_classe_${classData?.name}.pdf`;
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
                document.body.removeChild(a);
            ...
            console.error('Erreur impression cartes classe:', err);
            alert('Erreur lors de l\'impression des cartes de la classe');
        }
    };
    ...
    const percentage = students.length > 0 ? Math.round((studentsWithPhoto / students.length) * 100) : 0;
