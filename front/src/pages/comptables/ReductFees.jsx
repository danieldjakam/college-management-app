...
const ReductFees = () => {
	...
	const handleUpdate = (e) => {
		...
		let diff = [
			{
				name: "inscription",
				value: student.inscription - studentCopy.inscription
			},
			{
				name: "first_tranch",
				value: student.first_tranch - studentCopy.first_tranch
			},			
			{
				name: "second_tranch",
				value: student.second_tranch - studentCopy.second_tranch
			},			{
				name: "third_tranch",
				value: student.third_tranch - studentCopy.third_tranch
			},		
			{
				name: "graduation",
				value: student.graduation - studentCopy.graduation
			},			
			{
				name: "assurance",
				value: student.assurance - studentCopy.assurance
			},
		]
		student.diff = diff;
		if (st < ft) {
			Swal.fire({
				...
				fetch(host+'/users/confirmAdminPassword', 
					...
					.then(res => {
						console.log(res);
						if (res.success) {
							fetch(host+'/students/'+studentToEditId, {method: 'PUT', body: JSON.stringify(student), 
			...
			fetch(host+'/students/'+studentToEditId, {method: 'PUT', body: JSON.stringify(student), headers: {'Content-Type': 'application/json', 'Authorization': sessionStorage.user}})
				...
				.then(res => {
					if (res.success) {
						...
						window.open(`${host}/download/recu2/${studentToEditId}/${t}/${JSON.stringify(diff)}/${sessionStorage.user}`);
						window.open(`${host}/download/recu/${studentToEditId}/${t}/${JSON.stringify(diff)}/${sessionStorage.user}`);
						navigate(-1);
					}else{
						setError(res.message)
					}
									...
									return <tr>
										...
										<td>
											<a target={'_blank'} rel='noreferrer' 
												href={`${host}/download/recu/${p.recu_name}`} 
