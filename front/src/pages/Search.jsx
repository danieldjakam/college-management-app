...
function SearchView() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const resp = await fetch(`${host + "/students/getAll"}`, {
        headers: {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const resp = await fetch(host + "/teachers/getAll", {
        headers: {
          Authorization: sessionStorage.user,
        },
      });
  ...
  useEffect(() => {
    (async () => {
      ...
      const resp = await fetch(
        host + "/class/getOAll/" + sessionStorage.getItem("section_id"),
        {
          headers: {
            Authorization: sessionStorage.user,
          },
  ...
  const handleSearch = (column) => {
    ...
    switch (column) {
      case "t":
        const resultsTeachersByName = teachers.filter((teacher) =>
          teacher.name.toLowerCase().includes(searchValue.toLocaleLowerCase())
        );
        const resultsTeachersBySubname = teachers.filter((teacher) =>
          teacher.subname
            .toLowerCase()
            .includes(searchValue.toLocaleLowerCase())
        );
        const finallyTeachersResults = [
          ...new Set([...resultsTeachersByName, ...resultsTeachersBySubname]),
        ];
        setResults(finallyTeachersResults);
        break;

      case "s":
        const resultsStudentsByName = students.filter((student) =>
          student.name.toLowerCase().includes(searchValue.toLocaleLowerCase())
        );
        const resultsStudentsBySubname = students.filter((student) =>
          student.subname
            .toLowerCase()
            .includes(searchValue.toLocaleLowerCase())
        );
        const finallyStudentsResults = [
          ...new Set([...resultsStudentsByName, ...resultsStudentsBySubname]),
        ];
        setResults(finallyStudentsResults);
        break;

      case "c":
        const resultsClassesByName = classes.filter((c) =>
    ...
    <div className="container">
      ...
      >
        <input
          ...
          placeholder={searchTraductions[getLang()].placeholder}
        />
      ...
      {isQuestionSearch ? (
        <div className="optionsSearch">
          <div
            ...
            className={`${searchColumn === "s" ? "isthis" : ""} option`}
          >
          ...
          {sessionStorage.stat === "ad" ? (
            <>
              <div
                ...
                className={`${searchColumn === "t" ? "isthis" : ""} option`}
              >
              ...
              <div
                ...
                className={`${searchColumn === "c" ? "isthis" : ""} option`}
              >
      ...
      {loading ? (
        <div
          className="error"
          style={{ position: "absolute", top: "39%", left: "53%" }}
        >
          <ReactLoading color="#fff" type="spin" />
        </div>
      ) : results.length > 0 ? (
        <div className="allClas" style={{ marginTop: "20px" }}>
