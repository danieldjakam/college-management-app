...
const Cm2BE = ({type}) => {
    ...
    useEffect(() => {
        (
            async () => {
                ...
                data4.forEach(sub => {
                    tot += sub.over
                    const note = data5.filter(n => n.subject_id === sub.id.toString()).length > 0 ? parseFloat(data5.filter(n => n.subject_id === sub.id.toString())[0].value) : 0
                    if(sub.over / 2 > note){
                        bc.push(sub.name)
                    }
                })
    ...
    <nav className="navbar navbar-expand-lg" style={{padding: '10px 10px'}}>
        <a target={'_blank'} href={`${host}/download/pdf/bul5/${class_id}/${student_id}/${exam_id}`} className="btn btn-success">Télécharger le bulletin</a>
    </nav>
