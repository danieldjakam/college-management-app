...
const MatFrBT = ({type}) => {
    ...
    useEffect(() => {
        (
            async () => {
                ...
                let r = [];
                data4.forEach(dom => {
                  let d = data5.filter(t => t.domainId === dom.id.toString());
                  d.forEach(p => {
                    act.push(p)
                  })
                })
                ...
                data4.forEach(pp => {
                    data5.filter(oo => oo.domainId === pp.id.toString() ).forEach(ac => {
                        const note = data6.filter(n => n.activitieId === ac.id.toString()).length > 0 ? parseFloat(data6.filter(n => n.activitieId === ac.id.toString())[0].value) : 0;
                        if (parseInt(ac.appreciationsNber) === 4 && ( note !== 2 || note !== 1 )) {
                            ba.push(ac.name)
                        }
                        else if(parseInt(ac.appreciationsNber) === 3 && note !== 1){
                            ba.push(ac.name)
                        }
                    })
                })
    ...
    <nav className="navbar navbar-expand-lg" style={{padding: '10px 10px'}}>
        <a target={'_blank'} href={`${host}/download/pdf/bul2/${class_id}/${student_id}/${exam_id}`} className="btn btn-success">Télécharger le bulletin</a>
    </nav>
            ...
            <tbody>
                {
                    domains.map((domain, id) => {
                        const herActivities = activities.filter(act => act.domainId === domain.id.toString());
                        const herNotes = notes.filter(n => n.domain_id === domain.id.toString());
                        return <> 
                            <tr key={id}>
                            ...
                            </tr>
                            {
                                herActivities.map((act, id) => {
                                    const note = herNotes.filter(n => n.activitieId === act.id.toString()).length > 0 ? parseFloat(herNotes.filter(n => n.activitieId === act.id.toString())[0].value) : 0;
                                    return <tr key={id}>
                                        <td>{act.name}</td>
                                        <td>
