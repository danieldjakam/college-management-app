        ...
        <tbody>
            {
                students.length > 0 ? students.map((student, index) => {
                    const herNotes = notes.length > 0 ? notes.filter(n => n.student_id === student.id.toString()) : {}
                    let totalNote = 0;
                    return <tr key={index}>
                        <td>
                            {index + 1}
                        ...
                        {
                            mat.map((m, f) => {
                                const notesForThisMatiere = herNotes !== {} && herNotes.length > 0  ? herNotes.filter(h => h.sub_com_id === m.id.toString()) : {};
                                let total = 0;
                                const tags = JSON.parse(m.tags)
                                return <> 
                                            {
