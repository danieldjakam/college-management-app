    ...
    return <div className="container">
        ...
        <thead style={{color: '#000', fontSize :'1rem', fontWeight: '500'}}>
            ...
            <tr>
              ...
              {
                domains.length > 0 ? domains.map((domain, id) => {
                  const activities2 = activities.filter(ac => ac.domainId === domain.id.toString())
        ...
        <tbody>
            {
                students.length > 0 ? students.map((student, index) => {
                    const notesForStudents = notes.filter(n => n.student_id === student.id.toString()) ;
                    return <tr key={index}>
                        <td>
                            {index + 1}
                        </td>
                        <td>
                        ...
                        
                          {
                            domains.length > 0 ? domains.map((domain, id) => {
                              const activities2 = activities.filter(ac => ac.domainId === domain.id.toString())
                                return activities2.length > 0 ? activities2.map((activitie, id) => {
                                    const note = notesForStudents.filter(n => n.activitieId === activitie.id.toString()).length > 0 ? parseFloat(notesForStudents.filter(n => n.activitieId === activitie.id.toString())[0].value) : 0;
                                    return <td key={id}>
