module.exports.addTrimestre = async (req, res) => {
    ...
    if ( name && name !== '' ) {
        if (name.length < 5 || name.length > 30) {
            ...
            req.connection.query('INSERT INTO trims(id, name, seqIds, school_year) VALUES(?, ?, ?, ?)', 
                                [req.jwt.sign(name, req.env.SECRET), name, JSON.stringify(seqIds), req.school_year], (err, resp) => {
