...
module.exports.addStudent = (req, res) => {
    const {id} = req.params;
    let {name, subname, birthday ,fatherName, birthday_place, phone_number, profession, email, sex, isNew, status} = req.body;
    ...
    if (name) {
        ...
        if (name.length < 3) {
            ...
            [name, subname, req.params.id, sex, 
                ...
                sta, isNew, req.payload.school_id], (err, respp, fields) => {
...
module.exports.updateStudent = (req, res) => {
    ...
    const {name, subname ,fatherName, phone_number, profession, email, sex, 
        ...
        graduation, is_new, assurance, birthday} = req.body;
    ...
    if (name) {
        if (name.length < 3) {
            ...
            req.connection.query(`UPDATE students SET name = ?, subname = ?, sex = ?, email = ?, 
                                    phone_number = ?, fatherName = ?, profession = ?, birthday = ?, birthday_place = ?, 
                                    status = ?, inscription = ?, first_tranch = ?, second_tranch = ?, 
                                    third_tranch = ?, graduation = ?, assurance = ?, is_new = ? WHERE id = ? AND school_id = ?`, 
                                    [name, subname, sex, email, phone_number.toString(), 
                                        fatherName, profession, birthday, birthday_place, status, inscription, 
                                        first_tranch, second_tranch, third_tranch, graduation, assurance, is_new,
...
module.exports.getPayements = (req, res) => {
    req.connection.query(`SELECT p.amount, p.created_at, p.recu_name, u.email FROM payments p
...
module.exports.getSpecificStudents = (req, res) => {
    req.connection.query('SELECT year_school FROM settings WHERE school_id = ?', [req.payload.school_id], (rerr, respe) => {
        ...
        req.connection.query('SELECT * FROM students WHERE class_id = ? AND school_year = ? AND is_new = "no" AND school_id = ? AND school_year = ? ORDER BY name ASC', [req.params.id, year_school, req.payload.school_id, req.school_year] , (err, oldStudents) => {
            
            if(err) console.log(err);
            else {
                req.connection.query('SELECT * FROM students WHERE class_id = ? AND school_year = ? AND is_new = "yes" AND school_id = ? AND school_year = ?', [req.params.id, year_school, req.payload.school_id, req.school_year] , (err, newStudents) => {
                    let resp = [];
