module.exports.store = async (req, res) => {
    ...
    if ( name && name !== '' && slug && slug !== '' &&  section && section !== '' &&  comId && comId !== '' ) {
        if (name.length < 3 || name.length > 255) {
            ...
            req.connection.query(`INSERT INTO sub_com(id, name, slug, section, comId, tags, school_year) 
                                    VALUES(?, ?, ?, ?, ?, ?, ?)`, 
                                    [req.jwt.sign(name, req.env.SECRET), name, slug, 
                                        section, comId, JSON.stringify(tag), req.school_year], 
                                        (err, resp) => {
...
module.exports.all = (req : any, res : any) => {
    req.connection.query('SELECT * FROM sub_com AND school_year = ?', [req.school_year], (err, resp) => {
...
module.exports.some = (req : any, res : any) => {
    req.connection.query('SELECT * FROM sub_com WHERE comId = ? AND school_year = ?', [req.params.com_id, req.school_year], (err, resp) => {
        if(err) console.log(err);
        else res.status(201).json(resp);
    })
}
...
module.exports.show = (req : any, res : any) => {
    req.connection.query('SELECT * FROM sub_com WHERE id = ? AND school_year = ?', [req.params.id, req.school_year], (err, resp) => {
        if(err) console.log(err);
...
module.exports.update = (req : any, res : any) => {
    ...
    if ( name && name !== '' && section && section !== '' ) {
        if (name.length < 5) {
            ...
            req.connection.query('UPDATE sub_com SET name = ?, section = ? WHERE id = ? AND school_year = ? ', [name, section, req.params.id, req.school_year], (err, resp) => {
                if(!err) res.status(201).json({success: true})
                else console.log(err);
            })
        }
...
module.exports.delete = (req : any, res : any) => {
    const {id} = req.params;
    req.connection.query('DELETE FROM sub_com WHERE id = ? AND school_year = ?', [id, req.school_year], (err, resp) => {
