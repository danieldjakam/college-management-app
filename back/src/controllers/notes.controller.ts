					...
					if (parseFloat(value) <= over) {
					 req.connection.query('SELECT * FROM notes WHERE exam_id = ? AND student_id = ? AND sub_com_id = ? AND tag_name = ?', [exam_id, student_id, sub_com_id, tag_name], (err, resp) => {
						if (err) {
							...
							if (resp.length < 1) {
								req.connection.query('INSERT INTO notes(student_id, exam_id, class_id, sub_com_id, tag_name, value, school_year) VALUES(?, ?, ?, ?, ?, ?, ?)', [student_id, exam_id, class_id, sub_com_id, tag_name, val, req.school_year], (err2, resp2) => {
									if(!err2){
								...
								req.connection.query('UPDATE notes SET student_id = ?, exam_id = ?, class_id = ?, sub_com_id = ?, tag_name = ?, value = ? WHERE exam_id = ? AND student_id = ? AND sub_com_id = ? AND tag_name = ? AND school_year = ?', [student_id, exam_id, class_id, sub_com_id, tag_name, val, exam_id, student_id, sub_com_id, tag_name, req.school_year], (err2, resp) => {
									if(!err2){
										req.connection.query('SELECT * FROM notes WHERE exam_id = ? AND student_id = ? AND class_id = ?', [exam_id, student_id, class_id], (err3, notes) => {
											let totalPoints = 0;
											notes.forEach(note => {
...
module.exports.getNotes3 = async (req, res) => {
    ...
    req.connection.query('SELECT year_school FROM settings WHERE school_id = ?', [req.payload.school_id], (err, settings) => {
		const {year_school} = settings[0];
		req.connection.query('SELECT * FROM notesBySubject WHERE class_id = ? AND exam_id = ? AND school_year = ?', [req.params.class_id, req.params.exam_id, year_school, req.school_year], (err, resp) => {
			if(err) console.log(err);
			else res.status(201).json(resp)			
		})
	})
...
module.exports.getNotesByTrimPeoPle = async (req, res) => {
    req.connection.query('SELECT seqIds FROM trims WHERE id = ? AND school_year = ?', [req.school_year], [req.params.trim_id], (err, resp) => {
	   if(err) console.log(err);
	   else{
			const {seqIds} = resp[0]
			const seqs = JSON.parse(seqIds);
			const fEvalId = seqs[0];
			const sEvalId = seqs[1];
			req.connection.query('SELECT * FROM notes WHERE exam_id = ? AND student_id = ? AND school_year = ?', [fEvalId, req.params.student_id, req.school_year], (e, fEvalNotes) => {
				if(e) console.log(e);
				req.connection.query("SELECT * FROM notes WHERE exam_id = ? AND student_id = ? AND school_year = ?", [sEvalId, req.params.student_id, req.school_year], (e2, sEvalNOtes) => {
					if(e2) console.log(e2);
					let ar = [];
					fEvalNotes.forEach(n1 => {
						sEvalNOtes.forEach(n2 => {
